...
<?php include '../include/installed_judge.php'; ?>   <!-- 插入installed_judge.php文件 -->
<!-- 导航栏 -->
<?php include '../include/front_nav.php'; ?>        <!-- 插入front_nav.php文件 -->
...
//判断用户登录状态
if (!isset($_COOKIE["user"])) {  //如果从user获取到的cookie为空，则显示跳转
    echo '<div class="alert alert-warning" role="alert">请先登录哦~正在跳转</div>';
    header("refresh:1;url=student_login.php?action=login");
    exit();
} else {
    //登陆状态下判断行为
    if (@$_GET['action'] == 'start') {   //如果登录状态为start
        //开始答题
        date_default_timezone_set("Asia/Hong_Kong"); //设置脚本中所有日期/时间函数使用的默认时区
        $sql = "SELECT * FROM quiz_record WHERE md5_sno='" . $_COOKIE["user"] . "' AND summit_flag=0";  //定义一个变量用来存储sql语句，即从md5_sno中查询为quiz_record的行
        $result = $conn->query($sql);  //将sql执行后的结果存放进result
        //如果有未答完的试卷

        if ($result->num_rows > 0) {   //如果取出的行大于0
            while ($row = $result->fetch_assoc()) {  //则从结果集中取出一行作为row变量的值且该行不为空

                if ($row['dead_time'] < time()) {  //time()当前的 Unix 时间戳,如果查出来的结果中的dea_time()小于当前的Unix时间戳则，执行下面的语句

                    //如果超过答题时间
                    echo "时间已过哦,真遗憾";   //考试时间已过
                    $sql2 = "UPDATE quiz_record SET summit_flag=1 WHERE md5_sno='" . $_COOKIE["user"] . "' AND summit_flag=0";   //创建一个变量用来存放sql语句更新md5_sno表
                    $conn->query($sql2);   //将执的语句对象传给conn
                } else {
                    //未超时,继续作答
...
<div id="msg"></div>';
   ...
   <input type="submit" value="交卷" class="btn btn-primary">
            ...
            //倒计时功能
            $endtime = strtotime("+5 Minutes 5 Seconds");  //strtotime(),将任何英文文本日期时间描述解析为 Unix 时间戳
            $nowtime = time();   //返回当前的 Unix 时间戳
    ...
    if (@$_GET['action'] == 'judge') {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $per_score = $row['per_score'];
                $questions = $row['questions'];
                $quests = explode("*", $questions);  //按字符串拆分字符串
            }
        }
        $score = 0;
